...
# Изменение новости (PUT) или частичное (PATCH)
...
def edit_news(news_id):
    ...
    news = db_sess.get(News, news_id)
    if not news:
        return make_response(jsonify({'error': 'Новость отсуттсвует'}), 404)
# Список полей под редактирование
...
db_sess.commit()
return jsonify({'success': f'Поля новости {news.title} замененыэ)
